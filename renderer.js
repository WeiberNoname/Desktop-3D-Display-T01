...
// Helper function to update gear and close button positions dynamically
...
function setupSettingsUI() {
  ...
  if (closeBtn) {
    closeBtn.addEventListener('mouseenter', disableClickThrough);
  }
  ...
  if (resetSteamStatsBtn) {
    resetSteamStatsBtn.addEventListener('click', (event) => {
      event.preventDefault();
      const success = ipcRenderer.sendSync('reset-steam-stats');
      loadDiagnosticsLogs();
      if (success) {
        showSpeechBubble("🧹 Steam Cloud stats & achievements reset!", 4000);
      } else {
        showSpeechBubble("⚠️ Reset requested (Check Steam console).", 4000);
      }
    });
  }
}
